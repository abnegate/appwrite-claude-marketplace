...
- **Schema** — fluent table builder. Obtain a `Table` via `(new Schema\MySQL())->table('users')`, then chain typed column methods (`id`, `string`, `text`, `mediumText`, `longText`, `integer`, `bigInteger`, `serial`/`bigSerial`/`smallSerial`, `float`, `boolean`, `datetime`, `timestamp`, `json`, `binary`, `enum`, `point`, `linestring`, `polygon`, `vector`, `timestamps()`, ClickHouse-only `fixedString($name, $length)`). Each returns a `Column` exposing modifiers (`nullable`, `default`, `unsigned`, `unique`, `primary`, `autoIncrement`, `after`, `comment`, `collation`, `check`, `generatedAs` + `stored`/`virtual`, `ttl` + `lowCardinality()` + repeatable `codec('Delta(4)')`/`codec('LZ4')` (ClickHouse), `userType` (PostgreSQL)). Table-level fluent ops: `index()`, `uniqueIndex()`, `fulltextIndex()`, `spatialIndex()`, `foreignKey()`, `primary([...])` (composite PKs), `check()`, `partitionByRange/List/Hash()`, `engine()`, `orderBy()`, `ttl()`, `settings()`, `sampleBy()` (ClickHouse). Terminal `create()`/`createIfNotExists()`/`alter()`/`drop()`/`dropIfExists()`/`truncate()`/`rename()` returns a `Statement`. Per-dialect classes: `Schema\MySQL`, `MariaDB`, `PostgreSQL` (incl. SERIAL types, extensions, custom types, sequences, collations), `SQLite`, `ClickHouse` (10 engines, TTL, skip-index algorithms via `IndexAlgorithm` enum, table-level SETTINGS, `SAMPLE BY`, `LowCardinality`, `FixedString`, column-level `CODEC`), `MongoDB`.
- **Dialect features as interfaces** — `Schema/Feature/{Views, ReplaceView, Databases, RenameIndex, AnalyzeTable, Partitioning}` and `Builder/Feature/{Spatial, FullTextSearch, Upsert, InsertOrIgnore, UpsertSelect}`. Dialects opt in via `implements` + `use Schema/Trait/*` / `Builder/Trait/*`. **Methods only exist on the dialect class when the dialect supports them** — calling `dropTrigger()` on `Schema\SQLite` or `upsertSelect()` on `Schema\MongoDB` is now a compile-time error, not a deferred `UnsupportedException` at runtime. `Schema::table()` is abstract and each dialect returns its own covariant `Table\X` (`Table\ClickHouse` owns `engine`/`orderBy`/`ttl`/`settings`/`sampleBy`/`partitionBy(expr)`; `Table\Trait\*` composes `ForeignKeys`, `Checks`, `CompositePrimary`, `StandardPartitioning`, `FulltextSpatialIndex` where applicable).
...
- **Recently rehoused dialect-only knobs** — `lowCardinality()`, `fixedString($name, $length)`, `codec('…')`, and `sampleBy()` live exclusively on the ClickHouse `Column`/`Table` classes. PostgreSQL-only DDL methods (e.g. trigger DDL) emit `ON table_name` for `dropTrigger`. SQLite-specific gotchas: ALTER FK is unsupported (the `Feature\ForeignKeys` trait was dropped from the SQLite Table), and `Builder\SQLite` no longer inherits `filterDistance`/`filterSearch` (Spatial/FullTextSearch was lifted off the SQL abstract). MongoDB implements `Upsert` + `InsertOrIgnore` only — `upsertSelect()` is not in scope. PostgreSQL hash partitioning emits no `PARTITIONS N` clause
