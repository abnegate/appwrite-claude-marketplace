...
Test file detection is conservative — any staged path that looks like a
test counts:
  - `tests/` or `test/` anywhere in the path
  - filename contains `test` / `Test` / `_spec` / `.spec.`
  - PHPUnit: `Test.php` suffix
  - Kotlin/Java: `Test.kt`, `Test.java`
  - Rust: `#[test]` inline — we count any edit under `tests/` or
    `src/` that touches a `#[test]` block, which we can't detect from
    filenames alone. Conservatively: require a test-shaped path.
